...
import { adminResource } from "../../services/api";
import ImageUpload from "./ImageUpload";
import RichTextEditor from "./RichTextEditor";

const api = adminResource("blogs");

const empty = {
  order: "", title: "", category: "", date: "", readTime: "",
...
export default function AdminBlogs() {
  ...
  const openEdit = (b) => { setForm({ ...b, tags: Array.isArray(b.tags) ? b.tags.join(", ") : (b.tags || "") }); setEditId(b.id); setView("form"); };
  const cancel = () => { setView("list"); setForm(empty); setEditId(null); };
  const set = (key, val) => setForm((f) => ({ ...f, [key]: val }));

  const save = async () => {
    if (!form.title || !form.order) return alert("Title and Order are required.");
    setSaving(true);
    const { id, ...data } = form;  // id is server-assigned
  ...
  const del = async (docId) => {
    try { await api.remove(docId); await load(); } catch (e) { alert(e.message); }
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm p-6 max-w-xl mx-auto">
        ...
        <button onClick={save} disabled={saving}
          ...
          {saving ? "Saving..." : editId ? "Update Blog" : "Add Blog"}
        </button>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {blogs.map((b) => (
          <div key={b.id} className="bg-white rounded-xl p-5 shadow-sm flex flex-col gap-3">
            {b.image && (
